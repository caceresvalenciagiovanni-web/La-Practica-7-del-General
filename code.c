...
void div_() { Datum d1, d2, d3; d2 = pop(); d1 = pop(); d3.val = diviVector(d1.val, d2.val); liberaVector(d1.val); liberaVector(d2.val); push(d3); }
...
/* =========================================================
   FUNCIONES FALTANTES DE ENTRADA/SALIDA PARA HOC6
   ========================================================= */

extern FILE *fin; /* Apuntador al archivo o consola de entrada */
...
void varread(void) {
    ...
    /* Leemos un numero escalar desde la consola (o archivo) */
    if (fscanf(fin, "%lf", &temp) != 1) {
        temp = 0.0; /* Valor por defecto si hay un error de lectura */
    }
    
    /* Liberamos el vector viejo si existia y creamos uno nuevo con el valor leido */
    if (var->u.val != NULL) liberaVector(var->u.val);
    var->u.val = creaVector(1);
    var->u.val->vec[0] = temp;
    
    d.val = copiaVector(var->u.val);
    push(d);
}

void prstr(void) {
    /* Imprime directamente la cadena de caracteres desde la memoria de instrucciones */
    printf("%s", (char *)*pc++);
}
